...
CREATE INDEX planet_osm_line_label ON planet_osm_line USING GIST (way) WHERE name_it IS NOT NULL OR ref IS NOT NULL;
...
CREATE INDEX planet_osm_point_place ON planet_osm_point USING GIST (way) WHERE place IS NOT NULL AND name_it IS NOT NULL;
CREATE INDEX planet_osm_polygon_admin ON planet_osm_polygon USING GIST (ST_PointOnSurface(way)) WHERE name_it IS NOT NULL AND boundary = 'administrative' AND admin_level IN ('0', '1', '2', '3', '4');
...
CREATE INDEX planet_osm_polygon_name_it ON planet_osm_polygon USING GIST (ST_PointOnSurface(way)) WHERE name_it IS NOT NULL;
CREATE INDEX planet_osm_polygon_name_z6 ON planet_osm_polygon USING GIST (ST_PointOnSurface(way)) WHERE name_it IS NOT NULL AND way_area > 5980000;
